...
from token_configs import TOKEN
from helpers.back_to_menu_interface import BackToMenuInterface
...
from helpers.message_handlers.main_message_handler import MessageHandler
from helpers.sql_alchemy.adapter import SQLAlchemyAdapter
from helpers.json_api import JsonApi
from helpers.configs import json_name
from helpers.watering_time_checker import WateringTimeChecker
from helpers.processing_functions import change_flower_title, change_group_title, change_group_description, \
...
def add_flower_ask_photo_call_query(call: CallbackQuery) -> None:
    try:
        if 'BACK' in call.data:
            ...
            photo = PhotoProcessor().get_photo_object(adding_photo=False, user_id=call.from_user.id)
...
def add_flower_confirm_data_call_query(call: CallbackQuery) -> None:
    try:
        ...
        if 'confirm_flower_data' in call.data:
            ...
            photo = PhotoProcessor().get_photo_object(adding_photo=adding_photo, user_id=call.from_user.id)
...
def create_item_photo_messages_handler(message: Message) -> None:
    try:
        if JsonApi(message.from_user.id).set_flower_photo:
            ...
            bytes_photo = PhotoProcessor().save_photo_and_convert_it_to_bytes(photo=photo, user_id=message.from_user.id)
