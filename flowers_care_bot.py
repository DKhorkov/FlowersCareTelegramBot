...
def help_instruction(message):
    ...
    \nДля того, чтобы обновить время последнего полива растения, нажмите на кнопку "🔄 Обновить время полива".
...
def start(message):
    ...
    item3 = telebot.types.KeyboardButton("🔄 Обновить время полива")
    markup.add(item1)
    markup.add(item2)
    markup.add(item3)
...
def user_message(message):
    if message.chat.type == 'private':
        ...
        elif message.text == "🔄 Обновить время полива":
            bot.send_message(message.chat.id, "Пожалуйста, введите название растения:")
